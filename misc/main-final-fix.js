...
// Initialize controls - using OrbitControls for all devices to fix desktop black screen issue
let controls;
let controlType;

// Always use OrbitControls for consistent behavior
...
if (isTouchDevice) {
  console.log('Using OrbitControls on touch device');
} else {
  console.log('Using OrbitControls on desktop (FlyControls removed to fix black screen issue)');
}
...
window.addEventListener('resize', () => {
  ...
  // No need to reload on device type changes since we're always using OrbitControls
...
function animate() {
  ...
  // Only need to call update without delta for OrbitControls
  controls.update();
  ...
  // Force rendering even if there are no input events
