...
// Increased baseRadius by 2.5x to create a wider spatial distribution
...
const mapSharedWallets = () => {
  ...
  return sharedHolders.map((wallet, i) => {
    ...
    // Limit radius to within 2000 for shared wallets and cluster more tightly
    // Scaled up from 800 to 2000 to match the baseRadius increase
    const r = Math.min(2000, baseRadius * Math.log(normalizedIndex + 1.5) / 3);
...
const mapFartcoinWallets = () => {
  ...
  return fartcoinHolders.map((wallet, i) => {
    ...
    // Increased from 200 to 500 to maintain proportions with the larger baseRadius
    if (i % 5 === 0) {
      x += 500 * Math.sin(i);
      y += 500 * Math.cos(i);
    }
...
// Log that we're ready to map data when needed
console.log('positionMapper.js loaded - will map data when initialized');
console.log('Current data state before initialization call: ',
  `Fartcoin=${fartcoinHolders.length}, `,
  `Goat=${goatTokenHolders.length}, `,
  `Shared=${sharedHolders.length}`);
...
export function generateAllPoints() {
  ...
  console.log(`Data before generating points: Fartcoin=${fartcoinHolders.length}, Goat=${goatTokenHolders.length}, Shared=${sharedHolders.length}`);
  
  if (fartcoinHolders.length === 0 || goatTokenHolders.length === 0) {
    console.warn('WARNING: Missing wallet data when generating points!');
  }
  ...
  console.log(`Generated points: Shared=${sharedPoints.length}, Fartcoin=${fartcoinPoints.length}, Goat=${goatTokenPoints.length}`);
