...
function createStarfield() {
  ...
  for (let i = 0; i < starCount; i++) {
    ...
    positions[i3] = (Math.random() - 0.5) * 120000;
    positions[i3 + 1] = (Math.random() - 0.5) * 120000;
    positions[i3 + 2] = (Math.random() - 0.5) * 120000;
...
function createMediumSpheres() {
  ...
  const coreRadius = 8000; // Distance from core to medium spheres
  const mediumSpheres = [];
  ...
  for (let i = 0; i < totalSpheres; i++) {
    ...
    // Create the sphere geometry (smaller than core, larger than wallet nodes)
...
function createWalletNodes(mediumSpheres) {
  ...
  const walletNodes = [];
  const walletRadius = 2000; // Distance from medium sphere to wallet nodes
  ...
  mediumSpheres.forEach((mediumSphere, mediumIndex) => {
    ...
    for (let i = 0; i < nodesPerMediumSphere; i++) {
      ...
      const size = 50 + normalizedAmount * 100; // Size between 50 and 150
...
function handleJetpack(delta) {
  ...
  if (controls.jetpackActive) {
    controls.movementSpeed = 1000 * controls.jetpackBoostFactor;
  } else {
...
async function init() {
  ...
  if (success) {
    ...
    if (versionElement) {
      versionElement.textContent = 'v25';
    }
