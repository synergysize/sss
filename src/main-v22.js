...
let allNodes = []; // Track all nodes for galaxy swirl animation

// GALAXY SWIRL SETTINGS - NEW FOR V22
const GALAXY_SWIRL = {
  baseRotationSpeed: 0.05,        // Base rotation speed
  distanceInfluence: 0.00005,     // How much distance affects rotation speed
  verticalOscillation: 0.2,       // Vertical movement amount
  oscillationSpeed: 0.1,          // Speed of vertical oscillation
  randomVariation: 0.2,           // Random variation in speeds (0-1)
  preserveRelativePositions: true // Keep relative positions between nodes
};

// CAMERA BOUNDARY SETTINGS
const CAMERA_CONSTRAINTS = {
  farClippingPlane: 100000,      
  initialPosition: new THREE.Vector3(0, 300, 700), // Updated for v22 - better view of galaxy
  maxDistance: 50000,            
  resetDistance: 30000,          
  checkInterval: 30              
};
...
function initScene() {
  ...
  // Set background color - darker for better contrast with galaxy
...
function createStarfield() {
  ...
  stars.frustumCulled = true; // Enable frustum culling for stars
  scene.add(stars);
...
function createLevel2Cluster(parentPosition, parentScale, parentColor) {
  ...
  centralNode.frustumCulled = true; // Enable frustum culling
  ...
  for (let i = 0; i < 30; i++) {
    ...
    miniSphere.frustumCulled = true; // Enable frustum culling
  ...
  // Add initial properties for galaxy swirl
  miniClusterGroup.userData.initialPosition = parentPosition.clone();
  miniClusterGroup.userData.distanceToCenter = parentPosition.length();
  miniClusterGroup.userData.angleOffset = Math.random() * Math.PI * 2;
  miniClusterGroup.userData.verticalOffset = Math.random() * Math.PI * 2;
  miniClusterGroup.userData.rotationSpeed = GALAXY_SWIRL.baseRotationSpeed * 
    (1 - GALAXY_SWIRL.randomVariation + Math.random() * GALAXY_SWIRL.randomVariation);
  
  // Add to allNodes array for galaxy swirl animation
  allNodes.push(miniClusterGroup);
...
function createWalletPointCloud(pointsArray, groupName, color = 0xffffff) {
  ...
  pointsArray.forEach((point, index) => {
    ...
    sprite.userData = { 
      isLevel1Wallet: true, 
      parentIndex: index,
      initialPosition: new THREE.Vector3(point.x, point.y, point.z),
      distanceToCenter: new THREE.Vector3(point.x, point.y, point.z).length(),
      angleOffset: Math.random() * Math.PI * 2,
      verticalOffset: Math.random() * Math.PI * 2,
      rotationSpeed: GALAXY_SWIRL.baseRotationSpeed * 
        (1 - GALAXY_SWIRL.randomVariation + Math.random() * GALAXY_SWIRL.randomVariation)
    };
...
// NEW FOR V22: Galaxy swirl animation for all nodes
function updateGalaxySwirl(delta) {
  // Galaxy-wide time for consistent rotation
  const time = clock.getElapsedTime();

  allNodes.forEach(node => {
    if (node && node.userData && node.userData.initialPosition) {
      // Get initial position and parameters
      const initialPos = node.userData.initialPosition;
      const distanceToCenter = node.userData.distanceToCenter;
      const angleOffset = node.userData.angleOffset;
      const verticalOffset = node.userData.verticalOffset;
      
      // Calculate rotation speed based on distance (farther nodes rotate slower)
      const rotationSpeed = node.userData.rotationSpeed - 
        (distanceToCenter * GALAXY_SWIRL.distanceInfluence);
      
      // Current angle for this node (time-based for smooth motion)
      const angle = angleOffset + time * rotationSpeed;
      
      // Calculate swirl position
      if (GALAXY_SWIRL.preserveRelativePositions) {
        // Apply rotation to initial position vector for more natural motion
        const rotationMatrix = new THREE.Matrix4().makeRotationY(angle);
        const rotatedPosition = initialPos.clone().applyMatrix4(rotationMatrix);
        
        // Add vertical oscillation
        const verticalMovement = Math.sin(time * GALAXY_SWIRL.oscillationSpeed + verticalOffset) 
          * GALAXY_SWIRL.verticalOscillation * Math.min(distanceToCenter / 1000, 5);
        
        // Update position
        node.position.x = rotatedPosition.x;
        node.position.z = rotatedPosition.z;
        node.position.y = rotatedPosition.y + verticalMovement;
      } else {
        // Simplified approach: just rotate around Y axis
        ...
        node.position.y = initialPos.y + 
          Math.sin(time * GALAXY_SWIRL.oscillationSpeed + verticalOffset) 
          * GALAXY_SWIRL.verticalOscillation * Math.min(distanceToCenter / 1000, 5);
      }
    }
  });
...
async function setupVisualization() {
  try {
    ...
    // V22: Set a fixed initial camera position for better view of the galaxy
...
function animate() {
  ...
  // V22: Update galaxy swirl animation
  updateGalaxySwirl(delta);
