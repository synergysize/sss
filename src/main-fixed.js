...
function createLevel2Cluster(parentPosition, parentScale, parentColor) {
  ...
  centralNode.scale.set(parentScale * 0.546, parentScale * 0.546, 1); // Increased by additional 30% from previous 0.42
  centralNode.position.set(0, 0, 0); // Will be positioned relative to parent later
  miniClusterGroup.add(centralNode);
  
  // Create 30 smaller orbiting spheres for Level 2 (increased from 20)
  for (let i = 0; i < 30; i++) {
    // Calculate orbit position using spherical coordinates
    const radius = parentScale * 0.273; // Increased orbit radius by 30% to maintain spacing
    const theta = i * (Math.PI * 2 / 30); // Evenly distribute 30 spheres
    ...
    const miniScale = parentScale * 0.2184; // Increased by additional 30% from previous 0.168
...
function createWalletPointCloud(pointsArray, groupName, color = 0xffffff) {
  ...
  pointsArray.forEach((point, index) => {
    ...
    // Increased parent node size by 30% (from baseScale * 3 to baseScale * 3.9)
    ...
    // Cap Level 2 clusters to avoid performance issues (increased from 300 to 500 wallets)
    if (index < 500) {
      ...
      level2Cluster.userData = { 
        parentIndex: index,
        orbitRadius: scale * 1.092, // Increased by additional 30% to maintain spacing with larger nodes
...
// Main initialization function - FIXED TO BE ASYNC
async function initializeVisualization() {
  ...
  try {
    ...
    console.log("Data loaded successfully. Generating 3D points...");
    
    // Now generate the points after data is loaded
    generateAllPoints();
    ...
    console.log(`Point data ready: Shared=${sharedPoints.length}, Fartcoin=${fartcoinPoints.length}, Goat=${goatTokenPoints.length}`);
    ...
    // Calculate camera distance - adjusted to account for the larger spatial distribution
    // Increased from previous value to ensure we can see the full expanded structure
...
initializeVisualization().then(success => {
  if (success) {
    console.log("Visualization initialization completed. Starting animation loop.");
    animate();
  } else {
    console.error("Failed to initialize visualization. Please check console for errors.");
  }
});