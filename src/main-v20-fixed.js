...
// CAMERA BOUNDARY SETTINGS - ADDED FOR V20
...
function initScene() {
  ...
  // V20 FIX: Increased far clipping plane to 100,000 units (from 20,000)
  ...
  // V20 FIX: Set a reasonable initial camera position
...
function setupControls() {
  ...
  try {
    if (isTouchDevice) {
      ...
      // V20 FIX: Reduced movement speed to prevent flying too far too quickly
      controls.movementSpeed = 300; // Reduced from 400
      ...
      controls.damping = 0.3; // Increased from 0.2 for better stopping
      controls.gravity = 0.5;
      
      // V20 FIX: Add boundary property to constrain movement
...
function setupEventListeners() {
  ...
  // V20 FIX: Add home key to reset camera position
  window.addEventListener('keydown', function(event) {
    if (event.code === 'Home') {
      resetCamera();
...
// V20 FIX: Enhanced reset camera function to ensure good viewing position
...
// V20 FIX: New function to check if camera is too far from scene center
...
async function setupVisualization() {
  try {
    ...
    // V20 FIX: Limit camera distance to be within far clipping plane
    ...
    // V20 FIX: Position camera more conservatively
    camera.position.set(
      boxCenter.x, 
...
function animate() {
  ...
  if (controlType === 'Fly') {
    ...
    // V20 FIX: Check camera position boundary every N frames
    frameCounter++;
