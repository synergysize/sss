...
function createStarfield() {
  ...
  for (let i = 0; i < starCount; i++) {
    const i3 = i * 3;
    // Create stars randomly in a 50,000³ unit cube
    positions[i3] = (Math.random() - 0.5) * 50000;
    positions[i3 + 1] = (Math.random() - 0.5) * 50000;
    positions[i3 + 2] = (Math.random() - 0.5) * 50000;
...
async function setupVisualization() {
  try {
    ...
    // Ensure frustum culling is active for all point clouds
    sharedGroups.mainGroup.frustumCulled = true;
    fartcoinGroups.mainGroup.frustumCulled = true;
    goatTokenGroups.mainGroup.frustumCulled = true;
    sharedGroups.level2Group.frustumCulled = true;
    fartcoinGroups.level2Group.frustumCulled = true;
    goatTokenGroups.level2Group.frustumCulled = true;
...
function animate() {
  ...
  // Apply 10% gravity pull toward center (0, 0, 0)
  const pull = new THREE.Vector3(0, 0, 0).sub(camera.position).multiplyScalar(0.001);
